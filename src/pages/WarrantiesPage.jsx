...
function WarrantyCard({ canManage, onEdit, record }) {
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm sm:p-4">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-2.5 text-sm sm:grid-cols-4">
        <div className="min-w-0">
      ...
      {canManage && sale?.id && (
        <button
          className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-bold text-blue-700 transition hover:bg-blue-100 sm:w-auto"
...
function WarrantiesPage({ currentProfile }) {
  ...
  useEffect(() => {
    tabListRef.current
      ?.querySelector(`[data-warranty-status="${statusFilter}"]`)
  ...
  return (
    <div className="space-y-4 text-slate-950">
      ...
      <section className="snap-x overflow-x-auto rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm">
        <div
          aria-label="Warranty status filters"
          ...
          {statusFilters.map((filter) => {
            ...
            return (
              <button
                ...
                className={`inline-flex min-h-11 snap-start items-center justify-center gap-2 whitespace-nowrap rounded-xl px-3 py-2 text-sm font-bold transition ${
                  isActive
                    ? "bg-blue-600 text-white"
                    : "bg-slate-50 text-slate-600 hover:bg-slate-100"
                }`}
                data-warranty-status={filter.key}
                key={filter.key}
